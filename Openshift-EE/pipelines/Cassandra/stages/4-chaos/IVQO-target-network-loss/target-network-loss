...
pod() {
sshpass -p $pass ssh -o StrictHostKeyChecking=no $user@$ip -p $port 'cd e2e-openshift && bash Openshift-EE/pipelines/Cassandra/stages/4-chaos/IVQO-target-network-loss/target-network-loss node '"'$CI_JOB_ID'"'' '"'$CI_PIPELINE_ID'"' '"'$CI_COMMIT_SHA'"'
}
...
bash Openshift-EE/utils/pooling jobname:target-network-delay
bash Openshift-EE/utils/e2e-cr jobname:target-network-loss jobphase:Running init_time:"$current_time" jobid:"$job_id" pipelineid:"$pipeline_id" testcaseid:"$case_id"
...
-e 's/value: app-percona-ns/value: app-cass-ns/g'
-e 's/value: '\''enable'\''/value: '\'""\''/g' run_targetloss_test.yml
...
bash Openshift-EE/utils/event_updater jobname:target-network-loss $test_name jobid:"$job_id" pipelineid:"$pipeline_id" testcaseid:"$case_id"
...
bash Openshift-EE/utils/e2e-cr jobname:target-network-loss jobphase:Completed end_time:"$current_time" jobid:"$job_id" pipelineid:"$pipeline_id" testcaseid:"$case_id" test_result:$testResult

python3 Openshift-EE/utils/result/result_update.py $job_id $case_id $gitlab_stage 'Induce cstor target network loss and check if it gets recovered successfully' $testResult $pipeline_id "$current_time" $commit_id $gittoken

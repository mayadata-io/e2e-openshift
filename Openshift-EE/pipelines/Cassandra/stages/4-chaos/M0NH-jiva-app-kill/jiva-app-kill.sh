...
pod() {
sshpass -p $pass ssh -o StrictHostKeyChecking=no $user@$ip -p $port 'cd e2e-openshift && bash Openshift-EE/pipelines/Cassandra/stages/4-chaos/M0NH-jiva-app-kill/jiva-app-kill node '"'$CI_JOB_ID'"'' '"'$CI_PIPELINE_ID'"' '"'$CI_COMMIT_SHA'"'
}
...
bsah Openshift-EE/utils/pooling jobname:cstor-volume-mgmt-kill
...
---------------------------------------------------------------------------------------------------------------------|
| appLabel           | env   | app=jenkins-app                      | app=cassandra-jiva                             | 
...
| appNamespace       | env   | app-jenkins-ns                       | app-cass-ns-jiva                               | 
----------------------------------------------------------------------------------------------------------------------
EOF

sed -i -e 's/value: '\''app=jenkins-app'\''/value: '\''app=cassandra-jiva'\''/g' \
...
python3 Openshift-EE/utils/result/result_update.py $job_id $case_id 4-chaos "Induce application pod failure and check if it is recovered successfully" $testResult $pipeline_id "$current_time" $commit_id $gittoken
